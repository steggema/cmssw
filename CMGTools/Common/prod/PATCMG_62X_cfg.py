...
datasetInfo = (
    'CMS',
    # '/VBF_HToTauTau_M-125_14TeV-powheg-pythia6/Summer12-PU50_POSTLS161_V12-v1/AODSIM',
...
process.PATCMGJetCHSSequence = cms.Sequence(cloneProcessingSnippet(process, process.PATCMGJetSequence, 'CHS'))
...
# FIXME - JAN
# the default PAT sequence adds a 'PF' at the end, not sure where
# it comes from

process.impactParameterTagInfos.jetTracks = cms.InputTag('ak5JetTracksAssociatorAtVertex')
process.secondaryVertexTagInfos.jetTracks = cms.InputTag('ak5JetTracksAssociatorAtVertex')

process.pfNoJet.bottomCollection = cms.InputTag('particleFlowPtrs')

process.impactParameterTagInfosCHS.jetTracks = cms.InputTag('ak5JetTracksAssociatorAtVertexCHS')
process.secondaryVertexTagInfosCHS.jetTracks = cms.InputTag('ak5JetTracksAssociatorAtVertexCHS')
